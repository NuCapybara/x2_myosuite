...
with open("sensor_data.csv", mode="a") as file:
    ...
    while not glfw.window_should_close(window):
        ...
        # Visualize coordinate frame at the left knee joint
        left_knee_joint_pos = data.xpos[left_knee_joint_id]
        left_knee_joint_xmat = data.xmat[left_knee_joint_id].reshape(3, 3)

        mujoco.mjv_addGeoms(
            scene, model, data, mujoco.mjtCatBit.mjCAT_ALL.value, cam, opt, context
        )

        # Add coordinate frame at left knee joint position
        mujoco.mjv_addFrame(
            scene, left_knee_joint_pos, left_knee_joint_xmat, 0.1
        )  # 0.1 is the size of the frame
